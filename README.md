...
- Свой системный промпт на пользователя (`/system`) и прерывание генерации (`/stop`)
...
- История диалога с бюджетом контекста в токенах
...
- **Бюджет контекста считается в токенах приблизительно.** Ollama не отдаёт
  токенизатор, а тянуть его отдельно для каждой модели — лишняя зависимость и
  загрузка. Оценка намеренно завышает счёт: выбросить лишнее сообщение дешевле,
  чем недосчитать и получить молчаливое обрезание промпта внутри модели.
  Держите `MAX_CONTEXT_TOKENS` заметно ниже `num_ctx` модели.
